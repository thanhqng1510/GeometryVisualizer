...
public class SignUp extends JFrame implements ActionListener, MouseListener {
    ...
    public JPanel pressEmail(){
        ...
        textUser.setBounds(1,10,73,23);
        email.add(textUser);
        email.setBackground(new Color(0xF4F2F2));
        Font font= new Font("Roboto", Font.PLAIN,14);
        panel.setFont(font);
    ...
    public void actionPerformed(ActionEvent e)
    {
        ...
        {
            int flag = db.addNewUser(email.getText(), user.getText() , String.valueOf(password.getPassword()));
            if (flag == 0){
                setVisible(false);
                Login li= new Login();
            }else if(flag == 2){
                war.setText("Email has been registered");
                this.revalidate();
            }else if(flag == 1){
                war.setText("Email is invalid");
                this.revalidate();
            }
        }
